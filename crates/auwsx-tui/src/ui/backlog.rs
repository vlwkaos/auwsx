        ...
        .map(|b| {
            ListItem::new(Line::from(vec![
                Span::styled(
                    ...
                    Style::default().fg(approval_color(b.approval.as_str())),
                ),
                Span::styled(
                    format!("{:<7} ", b.source.as_str()),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::raw(b.text.clone()),
            ]))
        })
        .collect();
...
fn approval_color(approval: &str) -> Color {
    match approval {
        "approved" => Color::Green,
        "dismissed" => Color::DarkGray,
        _ => Color::Yellow, // pending
    }
}
