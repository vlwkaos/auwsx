//! Issue detail: header fields (left) + subtasks / findings / queue messages / log.
...
pub(super) fn render(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mut lines = vec![kv("id", &issue.id.to_string()), kv("title", &issue.title)];
    ...
    list_block(
        frame,
        left_rows[0],
        &format!("Plan checklist ({})", subtasks.len()),
        subtasks,
    );
    ...
    list_block(
        frame,
        left_rows[1],
        &format!("Review findings ({})", findings.len()),
        findings,
    );
    ...
    list_block(
        frame,
        left_rows[2],
        &format!("Queue messages ({})", steering.len()),
        steering,
    );
    ...
    log_block(frame, right_rows[1], app);
}
...
pub(super) fn log_block(frame: &mut Frame, area: Rect, app: &App) {
    log_block_inner(frame, area, app, None, false);
}
