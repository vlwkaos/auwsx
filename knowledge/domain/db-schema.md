...
slug: db-schema
kind: domain
title: auwsx SQLite schema (migrations, 11 tables)
description: The 11-table SQLite schema for the issue model — projects config, global Arsenal presets, issues, subtasks, findings, steering, backlog_items, routines, main_jobs, agent_runs, scheduler_runs — with CHECK domains, per-connection FK enforcement, and append-only/gate rules.
keywords: [0001_init.sql, 0003_arsenal_presets.sql, SQLite schema, CHECK domain, foreign_keys per connection, agent_runs append-only, steering append-only, backlog source approval gate, arsenal presets, projects config columns, issues columns, findings severity lens, routines writable_paths, main_jobs REJECTED scope_violation, scheduler_runs, completion_policy, merge_mode, review_max_rounds, conflict_max_attempts, db migration]
created: 2026-06-09
modified: 2026-06-09
---
...
`crates/auwsx-core/src/db/migrations/` — 11 tables, all enum-like
