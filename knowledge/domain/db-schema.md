...
| **projects** | per-project config | per-role agent cmds `main/plan/work/review_agent_cmd` (review NULL → fall back to work); `schedule_cron` (NULL/manual, `@tick`, five-field cron, `@every`); `completion_policy` (manual\|soft\|auto), `completion_soft_timeout_min`, `plan_gate_timeout_min`, `iteration_timeout_min`, `main_job_timeout_min`, `review_max_rounds`, `conflict_max_attempts`, `max_concurrency`, `merge_mode` (local\|pr), `skill_path`, `deepsleep_cron` |
...
| **issues** | pipeline unit | `status` (16-state CHECK), `branch`/`worktree_path` (set at PLANNING), `review_round`, `conflict_attempts`, `wait_until`, `has_pending_steering` |
